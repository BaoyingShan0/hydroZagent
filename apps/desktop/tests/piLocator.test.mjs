...
function loadPiLocatorModule(platform = process.platform, envOverrides = {}, homePath = tmpdir(), moduleOverrides = {}) {
	...
	const sandbox = {
		...
		if (id === "electron") {
				return { app: { getPath: () => homePath, isPackaged: false } };
			}
			return require(id);
		},
	};
...
test("prefers the integrated community-hydroagent CLI over a global pi installation", () => {
	...
	writeFileSync(integratedCli, "#!/usr/bin/env node\n", "utf8");

	try {
		const { PiLocator } = loadPiLocatorModule(
			"win32",
			{
				HYDROZAGENT_PI_CLI_PATH: integratedCli,
				HYDROZAGENT_NODE_EXECUTABLE: process.execPath,
			},
			root,
		);
		const locator = new PiLocator();
		assert.equal(locator.resolveCommand(), integratedCli);

		const invocation = locator.createInvocation(integratedCli, ["--mode", "rpc"]);
		assert.equal(invocation.command, process.execPath);
		assert.deepEqual(Array.from(invocation.args), [integratedCli, "--mode", "rpc"]);
		assert.equal(invocation.shell, false);
	} finally {
		rmSync(root, { recursive: true, force: true });
	}
