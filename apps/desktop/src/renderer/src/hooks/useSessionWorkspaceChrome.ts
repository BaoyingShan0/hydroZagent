...
export function useSessionWorkspaceChrome(options: {
  ...
  useEffect(() => {
    ...
    // 通知聚焦 IPC 仍位于历史 pet namespace，但必须经 desktopApi 适配，
    // 这样浏览器预览/内网 Web 不会因没有 Electron preload 而崩溃。
    const unsubscribe = desktopApi.pet.onFocusTarget(({ sessionId }) => {
      focusBySessionId(sessionId);
    });
    // 冷启动/页面加载期间点击通知：主进程直接 send 会在监听注册前丢失，
    // 挂载后主动拉取一次 pending 目标（与事件推送共用同一解析/重试逻辑）。
    void desktopApi.pet.getPendingFocusTarget?.().then((target) => {
