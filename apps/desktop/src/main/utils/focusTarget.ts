 ...
 * - hydrozagent://session/<uuid>：主路径。通知创建时直接嵌入会话 id（SessionRecord.id 跨重启稳定），
 *   冷启动/运行中均可跳转，不依赖 agent 运行时状态。
 * - hydrozagent://agent/<uuid>：兼容旧版 toast 的兜底格式（agent 运行时才可解析到会话）。
 * 返回 undefined 表示本次唤起不是通知点击，仅聚焦窗口即可。
 */
const FOCUS_UUID = "[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}";
const SESSION_RE = new RegExp(`hydrozagent://session/(${FOCUS_UUID})`, "i");
const AGENT_RE = new RegExp(`hydrozagent://agent/(${FOCUS_UUID})`, "i");
