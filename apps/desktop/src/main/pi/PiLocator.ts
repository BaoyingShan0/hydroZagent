...
export class PiLocator {
  ...
  resolveCommand(customPath?: string, wslEnabled?: boolean, wslDistro?: string, wslUser?: string) {
    ...
    // hydroZagent is distributed as one product: prefer the Agent runtime shipped
    // with this repository/application over an unrelated global pi installation.
    const integratedCommand = this.getIntegratedCommand();
    if (integratedCommand) return integratedCommand;
  ...
  createInvocation(command: string, args: string[], options: { wslCwd?: string } = {}): PiCommandInvocation {
    ...
    // Development uses the monorepo's built CLI JavaScript directly. Use the
    // parent Node executable explicitly because process.execPath is Electron here.
    if (command.toLowerCase().endsWith(".js") && existsSync(command)) {
      const nodeCommand = process.env.HYDROZAGENT_NODE_EXECUTABLE || "node";
      return {
        command: nodeCommand,
        args: [command, ...args],
        shell: false,
        pathPrefix: this.getCommandBinDir(nodeCommand),
      };
    }
  ...
  private getIntegratedCommand(): string | undefined {
    const developmentCommand = process.env.HYDROZAGENT_PI_CLI_PATH?.trim();
    if (developmentCommand && existsSync(developmentCommand)) return developmentCommand;

    if (!app.isPackaged) return undefined;
    const executableName = process.platform === "win32" ? "pi.exe" : "pi";
    const bundledCommand = join(process.resourcesPath, "pi-runtime", executableName);
    return existsSync(bundledCommand) ? bundledCommand : undefined;
  }
