...
// 浙水版不装配桌面宠物，因此无需为浮窗强制 XWayland。
applyLinuxDisplayBackendWorkaround(false);
...
if (process.platform === "win32") {
	const devAppId =
		devUserDataDirName === DEFAULT_DEV_USER_DATA_NAME
			? "com.hydrozagent.dev"
			: `com.hydrozagent.dev.${sanitizeDevBranchSegment(devGitBranch ?? "detached")}`;
	app.setAppUserModelId(isDevBuild ? devAppId : "com.hydrozagent.app");
}

// 注册 hydrozagent:// 自定义协议：系统通知点击（toast activationType="protocol"）通过该协议唤起应用，
// 唤起实例的 argv 携带 hydrozagent://session/<id> URL，主进程据此跳转对应会话。
...
if (app.isPackaged) {
	app.setAsDefaultProtocolClient("hydrozagent");
}
...
const RELEASES_URL = "https://github.com/BaoyingShan0/PiDeck/releases";
const LATEST_RELEASE_API =
	"https://api.github.com/repos/BaoyingShan0/PiDeck/releases/latest";
 ...
 * 若唤起源自「点击系统通知」（argv 携带 hydrozagent:// URL），额外向 renderer 发送聚焦目标，
...
function printStartupInfo() {
	...
	mainWindow.webContents.executeJavaScript(`
		...
		console.log("%c  https://github.com/BaoyingShan0/PiDeck/issues", "color: #3b82f6; text-decoration: underline;");
		console.log("");
...
function shouldShowMainWindowImmediately() {
	return isUsingLinuxXWaylandWorkaround(false);
}
...
function registerIpc() {
	...
	registerSystemIpc({
		...
		// 兼容历史 settings 字段；浙水版不再装配桌面宠物进程。
		reactToPetSettings: async () => undefined,
...
// 换肤背景图协议：内部 scheme 保留历史名以兼容已保存的背景资源 URL。
...
app.whenReady().then(async () => {
	...
	// hydroZagent 轻量发行版只装配 pi 网关；DSH 兼容代码不进入运行时路由。
	...
	// hydroZagent 轻量发行版不再后台预热 DSH；核心运行时统一走 pi。
	...
	// argv 携带 hydrozagent:// URL，窗口就绪后跳转对应会话。
	...
	void projectStore
		...
		.then(async () => {
			...
			broadcastVisibleProjects();
			// 浙水版不扫描或导入 DSH_HOME，避免启动阶段额外磁盘与后台开销。
		})
		.catch(() => undefined);
